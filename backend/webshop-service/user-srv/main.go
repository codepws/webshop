...
import (
	...
	"webshop-service/user-srv/steup"
...
func main() {
	...
	appConfig := config.AppConfig{}
	// 用户服务 - 配置中心初始化
	configmgr.InitConfig("user-srv", isDebug, &appConfig, onChange)
	...
	fmt.Println("ServerConfig:", *(appConfig.ServerConfig))
	...
	//数据库初始人
	err := global.DBMgr.Init(appConfig.DBs)
	if err != nil {
		panic(err)
	}
