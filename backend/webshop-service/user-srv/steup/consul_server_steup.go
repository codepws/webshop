...
func SteupServiceReg() (*servicereg.ConsulServiceRegistry, error) {
	...
	service_name := global.AppConfig.ServerConfig.Name //"user-srv"
	service_ip := global.AppConfig.ServerConfig.Host
	service_port, err := utils.GetFreePort() //动态获取空闲服务端口
	if err != nil {
		return nil, err
	}
	service_tags := global.AppConfig.ServerConfig.Tags // []string{"srv", "user"}
