...
import (
	...
	"webshop-service/order-srv/steup"
...
func main() {
	...
	appConfig := config.AppConfig{}
	// 用户服务 - 配置中心初始化
	configmgr.InitConfig("order-srv", isDebug, &appConfig, onChange)
	...
	fmt.Println("RocketMQ.OrderCreateInvReback:", *(appConfig.RocketMQ.OrderCreateInvReback))
	fmt.Println("RocketMQ.OrderPayInvReback:", *(appConfig.RocketMQ.OrderPayInvReback))
	fmt.Println("RocketMQ.OrderPayTimeout:", *(appConfig.RocketMQ.OrderPayTimeout))

	fmt.Println("GoodsSrv:", *(appConfig.GoodsSrv))
	fmt.Println("Inventory:", *(appConfig.InventorySrv))
...
func Chdir() (err error) {
	...
	index := strings.LastIndex(exPath, string(os.PathSeparator))
	newdir := exPath[:index]
	err = os.Chdir(newdir)
	if err != nil {
		log.Fatal(err)
