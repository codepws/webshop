...
func SteupRocketMQReg(RocketMQ *settings.RocketMQ) error {
	...
	log.Println("MQ订单支付超时消息消息者：", RocketMQ.OrderPayTimeout.CGroupName, RocketMQ.OrderPayTimeout.Topic, RocketMQ.OrderPayTimeout.Tag)
	//订单支付超时 Consumer
	cer, err := rocketmq.NewPushConsumer(
		consumer.WithGroupName(RocketMQ.OrderPayTimeout.CGroupName),
	...
	topic := global.AppConfig.RocketMQ.OrderPayTimeout.Topic
	tag := global.AppConfig.RocketMQ.OrderPayTimeout.Tag
	selector := consumer.MessageSelector{
		Type:       consumer.TAG,
		Expression: tag,
	}
	err = cer.Subscribe(topic, selector, servicer.MQOrderPayTimeoutCallback)
	if err != nil {
