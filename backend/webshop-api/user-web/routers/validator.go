...
func SetupValidator() {
	...
	if v, ok := binding.Validator.Engine().(*validator.Validate); ok {
		err := v.RegisterValidation("isMobilePhone", isMobilePhone)
		if err != nil {
			fmt.Println("[SetupValidator] 【验证输入手机号码】 注册失败")
		}
		err = v.RegisterValidation("IsTelephone", IsTelephone)
		if err != nil {
			fmt.Println("[SetupValidator] 【验证电话号码】 注册失败")
		}
	}
